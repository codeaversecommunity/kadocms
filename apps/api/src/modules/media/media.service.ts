...
export class MediaService {
  ...
  async uploadBase64(
    ...
    try {
      // Upload to Cloudinary
      const cloudinaryResult = await this.cloudinaryService.uploadBase64(base64Data, {
        folder: `kadocms/${workspace.slug}`,
      });
  ...
  async findAll(queryDto: QueryMediaDto, userId: string): Promise<PaginatedResult<any>> {
    const { workspace_id, page = 1, limit = 10, search, media_type, sortBy = 'created_at', sortOrder = 'desc' } = queryDto;
  ...
  async generateTransformationUrl(
    ...
    const transformedUrl = this.cloudinaryService.generateTransformationUrl(publicId, transformations);
